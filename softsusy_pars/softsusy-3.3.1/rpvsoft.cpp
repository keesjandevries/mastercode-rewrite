...
/** \File rpvsoft.cpp
   ...
   \brief File for RP violating MSSM object including all (real)
...
void RpvSoftsusy::rewsb(int sgnMu, double mt, double muOld) {
...
void RpvSoftsusy::isawigInterface764(char herwigInputFile [80], 
  ...
  for (i = 1; i<=3; i++) 
    for (l = 1; l<=3; l++) {
      ...
      if (l == 2) { j = 1; k = 3; }
      if (l == 3) { j = 2; k = 3; }
      if (displayLambda(LE).display(i, j, k) != 0.0) test = true;
    }

  if (test) softOutput << "y\n" << displayLambda(LE).display(1, 1, 2) << " " 
		 << displayLambda(LE).display(2, 1, 2) << " " 
		 << displayLambda(LE).display(3, 1, 2) << " " 
		 << displayLambda(LE).display(1, 1, 3) << " " 
		 << displayLambda(LE).display(2, 1, 3) << " " 
		 << displayLambda(LE).display(3, 1, 3) << " " 
		 << displayLambda(LE).display(1, 2, 3) << " " 
		 << displayLambda(LE).display(2, 2, 3) << " " 
		 << displayLambda(LE).display(3, 2, 3) << endl; 
  else softOutput << "n\n";
  ...
  for (i = 1; i<=3; i++) 
    for (j = 1; j<=3; j++) 
      for (k = 1; k<=3; k++) {
	softOutput << displayLambda(LD).display(k, i, j) << " ";
    }
  softOutput << endl;
  ...
  for (i = 1; i<=3; i++) 
    for (l = 1; l<=3; l++) {
      ...
      if (l == 2) { j = 1; k = 3; }
      if (l == 3) { j = 2; k = 3; }
      if (displayLambda(LU).display(i, j, k) != 0.0) test = true;
    }

  if (test) softOutput << "y\n" << displayLambda(LU).display(1, 1, 2) << " " 
		 << displayLambda(LU).display(1, 1, 3) << " " 
		 << displayLambda(LU).display(1, 2, 3) << " " 
		 << displayLambda(LU).display(2, 1, 2) << " " 
		 << displayLambda(LU).display(2, 1, 3) << " " 
		 << displayLambda(LU).display(2, 2, 3) << " " 
		 << displayLambda(LU).display(3, 1, 2) << " " 
		 << displayLambda(LU).display(3, 1, 3) << " " 
		 << displayLambda(LU).display(3, 2, 3) << endl; 
  else softOutput << "n\n";
...
void RpvSoftsusy::sparticleThresholdCorrections(double tb, 
  ...
  /// CMSSM 
  if (susyRpvBCatMZ) RpvSoftsusy::rpvSet(pars);
}
...
void RpvSoftsusy::outputGutScaleNonZeroInputs(ostream & out) {
  ...
  for (i=1; i<=3; i++)
	...
	if (fabs(displayLambda(LE).display(k, i, j)) > 6.66e-66) {
	    if (!anyCoups) { 
	      anyCoups = true;
	      out << "Block RVLAMLLEIN           # GUT scale LLE couplings \n";
	    }
	    out << "  " << i << " " << j << " " << k 
  ...
  for (i=1; i<=3; i++)
	...
	if (fabs(displayLambda(LD).display(k, i, j)) > 6.66e-66) {	
	    if (!anyCoups) { 
	      anyCoups = true;
	      out << "Block RVLAMLQDIN           # GUT scale LLE couplings \n";
	    } 
	    out << "  " << i << " " << j << " " << k 
  ...
  for (i=1; i<=3; i++)
	...
	if (fabs(displayLambda(LU).display(i, j, k)) > 6.66e-66) {
	    if (!anyCoups) { 
	      anyCoups = true;
	      out << "Block RVLAMUDDIN          # GUT scale LQD couplings \n";
	    } 
	    out << "  " << i << " " << j << " " << k 
...
void RpvSoftsusy::slha1(ostream & out, const char model[], 
  ...
  RpvSoftsusy kw;
  kw.rpvSet(pars);
  kw.outputGutScaleNonZeroInputs(out);
  ...
  //output RPV couplings LLE, LQD, UDD
  out << "Block RVLAMLLE Q= " << MZ <<  " # R-Parity violating LLE couplings \n";
  int k;
  for (i=1; i<=3; i++)
    for (j=1; j<=3; j++) 
      for (k=1; k<=3; k++){
  ...
  out << "Block RVLAMLQD Q= " << MZ <<  " # R-Parity violating LQD couplings \n";
  for (i=1; i<=3; i++)
    for (j=1; j<=3; j++) 
      for (k=1; k<=3; k++){
	out << "  " << i << " " << j << " " << k << "   "; 
  ...
  out << "Block RVLAMUDD Q= " << MZ <<  " # R-Parity violating UDD couplings \n";
  for (i=1; i<=3; i++)
    for (j=1; j<=3; j++) 
      for (k=1; k<=3; k++){
	out << "  " << i << " " << j << " " << k << "   "; 
  ...
  out << "Block RVT Q= " << MZ <<  " # R-Parity violating LLE soft terms \n";
  for (i=1; i<=3; i++)
    for (j=1; j<=3; j++) 
      for (k=1; k<=3; k++){
	out << "  " << i << " " << j << " " << k << "   "; 
  ...
  out << "Block RVTP Q= " << MZ <<  " # R-Parity violating LQD soft terms \n";
  for (i=1; i<=3; i++)
    for (j=1; j<=3; j++) 
      for (k=1; k<=3; k++){
	out << "  " << i << " " << j << " " << k << "   "; 
  ...
  out << "Block RVTPP Q= " << MZ <<  " # R-Parity violating UDD soft terms \n";
  for (i=1; i<=3; i++)
    for (j=1; j<=3; j++) 
      for (k=1; k<=3; k++){
  ...
  out << "Block RVKAPPA Q= " << MZ <<  " # R-Parity violating kappa \n";
  for (i=1; i<=3; i++){
    out << "     " << i << "   "; printRow(out, displayKappa().display(i));
    out << "   # kappa_{" << i << "}\n";      
  }
  
  out << "Block RVD Q= " << MZ <<  " # R-Parity violating D \n";
  for (i=1; i<=3; i++){
    out << "     " << i << "   "; printRow(out, displayDr().display(i));
    out << "   # D_{" << i << "}\n";      
  }
  
  out << "Block RVSNVEV Q= " << MZ <<  " # sneutrino VEVs D \n";
  for (i=1; i<=3; i++){
    out << "     " << i << "   "; printRow(out, displaySneutrinoVevs().display(i));
    out << "   # SneutrinoVev_{" << i << "}\n";      
  }
  
  out << "Block RVM2LH1 Q= " << MZ <<  " # M2LH1 \n";
  for (i=1; i<=3; i++){
  ...
  // output neutrino-neutralino mixing matrix  
    out << "Block RVNMIX Q= " << MZ <<  " # neutrino-neutralino mixing matrix \n";
  for (i=1; i<=7; i++)
    for (j=1; j<=7; j++) {
      out << "  " << i << " " << j << "   ";
...
void rpvSugraBcs(MssmSoftsusy & m, const DoubleVector & inputParameters) {
  ...
  if (!susyRpvBCatMZ) m.rpvSet(inputParameters);
