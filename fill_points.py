...
import numpy as np
import Storage.sqlite_storage as mcsql

def parse_args():
    parser = argparse.ArgumentParser(description='''Script that takes pickled points and puts them into sql database.
            The points table works, the collection table needs more work''')
...
#FIXME: may should move this function to Storage/sqlite_storage.py
...
def mc_points_to_rows(con,cur,points,collection_id=1):
    ...
    obs_columns, mc_obs_ids = mcsql.get_columns_and_observable_ids(con,cur)
    rows=[]
    for point in points:
