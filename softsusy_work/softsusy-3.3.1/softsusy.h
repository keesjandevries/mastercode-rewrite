...
public:
  ...
  */
  ...
  void slha1(ostream & out, const char model[], const DoubleVector & pars, 
	     ...
	     double mgut, bool altEwsb = false);
...
inline MssmSoftsusy::MssmSoftsusy()
  : SoftParsMssm(), physpars(), forLoops(), 
    ...
    mw(0.0), dataSet(), fracDiff(1.), setTbAtMX(false) { 
      setPars(110);
      setMu(0.0);
      setLoops(0);
      setThresholds(0);
...
inline MssmSoftsusy::MssmSoftsusy(const MssmSoftsusy & s)
  : SoftParsMssm(s.displaySoftPars()), physpars(s.displayPhys()), 
    ...
    mw(s.mw), dataSet(s.displayDataSet()), fracDiff(1.), setTbAtMX(false) {
    setPars(110);
    setMu(s.displayMu()); 
...
class MssmSoftsusyAltEwsb: public MssmSoftsusy {
private:
...
public:
  ...
  inline double displayMaCond() { return mAcond; };
  inline double displayMuCond() { return muCond; };

  inline void setMaCond(double maInput) { mAcond = maInput; };
  inline void setMuCond(double muInput) { muCond = muInput; };
  //  inline void setConditionStyle(int scs) { conditionStyle = scs; };

  /// Just sets the mu parameter to what the user wanted
  virtual void rewsbTreeLevel(int sgnMu) { setSusyMu(muCond); };
  ...
  virtual void rewsb(int sgnMu, double mt, double muOld = -6.66e66);
  /// Sets mu(MSUSY)=inputs(1) and mApole=inputs(2)
  virtual void setEwsbConditions(const DoubleVector & inputs);
};
