...
void MssmSoftsusy::rewsb(int sgnMu, double mt, double muOld) {
...
(void (*boundaryCondition)(MssmSoftsusy &, const DoubleVector &),
 ...
 oneset, bool gaugeUnification, bool ewsbBCscale) {
  ...
  try {
    ...
    runto(mz);
    if (susyRpvBCatMZ) sparticleThresholdCorrections(tanb, pars);
...
void MssmSoftsusy::sparticleThresholdCorrections(double tb, const DoubleVector & pars) {
...
void MssmSoftsusy::minparSLHA(ostream & out, const char model [], 
  ...
  else if (!strcmp(model, "nonUniversal")) extparSLHA(out, pars, altEwsb);
  else {
    ostringstream ii;
...
void MssmSoftsusy::lesHouchesAccordOutput(ostream & out, const char model[], 
					  ...
					  bool altEwsb) {
  if (forceSlha1 == true) {
    slha1(out, model, pars, sgnMu, tanb, qMax, numPoints, mgut, altEwsb);
    return;
  }
  ...
  sminputsSLHA(out); 
  minparSLHA(out, model, pars, tanb, sgnMu, altEwsb);
...
/** Start of MssmSoftsusyAltEwsb object **/
void MssmSoftsusyAltEwsb::rewsb(int sgnMu, double mt, double muOld) {
...
void MssmSoftsusyAltEwsb::setEwsbConditions(const DoubleVector & inputs) {
  muCond = inputs.display(1);
  mAcond = inputs.display(2);
  return; 
}
