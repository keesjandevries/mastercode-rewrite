...
void FlavourMssmSoftsusy::extparSLHA(ostream & out, 
    ...
    if (!altEwsb) {
      ...
      out << "   # mHu^2(MX)" << endl;    
    } else throw("Not yet implemented alternative EWSB conditions with FV\n");
...
void FlavourMssmSoftsusy::calcDrBarPars() {
  ...
  for(i=1; i<=3; i++) {
    ...
    s.me(1, i) = eSqMasses(i);
    s.me(2, i) = eSqMasses(i+3);
  ...
  if (mAsq < 0.0) {
    ...
    if (close(displayMu(), MZ, tol)) {
      double mApole = displayPhys().mA0; // physical value
  ...
  int pos;
  s.mh0 = temp.min(pos); s.mH0 = temp.max(); 
  s.mA0 = sqrt(mAsq); s.mHpm = sqrt(mAsq + mw2);
