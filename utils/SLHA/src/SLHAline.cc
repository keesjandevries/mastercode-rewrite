...
uint32_t SLHAline::GetFullIndex() const {
  return mIndexInfo; //method to get the full index
}

void SLHAline::SetFullIndex(int64_t index) {
...
void SLHAline::SetNumIndices(int64_t numindices) {
  ...
  if(numindices <= 0x3 && numindices >= 0 ) {
    ...
    uint32_t newindexinfo = mIndexInfo;
    newindexinfo = (newindexinfo & 0x3FFFFFFF); //mask off the top 2 bits
    uint32_t numindex = numindices << 30; //move the numindices to the top 2 bits
...
uint32_t SLHAline::GetIndex1() const {
  return (mIndexInfo & 0x00FFFFFF); //method to return Index1 from 32 bits
}

void SLHAline::SetIndex1(int64_t index1) {
  //do some range checking on the index passed to prevent debugging nightmares
...
uint32_t SLHAline::GetIndex2() const {
  return ((mIndexInfo & 0x3F000000) >> 24); //method to return Index2 from 32 bits
}

void SLHAline::SetIndex2(int64_t index2) {
  //do some range checking on the index passed to prevent debugging nightmares
  if(index2 <= 0x3F && index2 >= 0 ) {
    ...
    uint32_t ind2 = index2 << 24; //move index 2 to occupy bits 24-30
...
double SLHAline::GetValue() const {
  return mValue; //method to get the value of a line
}

void SLHAline::SetValue(double value) {
